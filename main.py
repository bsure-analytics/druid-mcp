...
def parse_key_value_pairs(pairs: str) -> dict[str, str]:
    ...
    return {k: v for k, v in (e.split("=", 1) for e in pairs.split()) if k and v}
...
def _enhance_schema_items(items: list, type_mapping: dict[str, str], default_type: str) -> list[dict[str, Any]]:
    ...
    """
    return [
        {"name": item, "type": type_mapping.get(item, default_type)}
...
async def _check_leader_service(cluster: str, endpoint: str, ctx: Context) -> dict[str, Any]:
...
@mcp.tool()
async def list_datasources(cluster: str, ctx: Context, include_details: bool = False) -> list[Any]:
    """List all available datasources in the cluster

    Args:
        cluster: Target cluster name (e.g., 'fret-dev', 'fret-prod')
        include_details: If True, includes segment counts and size info per datasource

    Returns:
        List of datasource names or datasource objects with metadata
    ...
    return await _make_request(cluster, "GET", "/druid/coordinator/v1/datasources", ctx, params=params)


@mcp.tool()
async def get_datasource_schema(cluster: str, datasource: str, ctx: Context) -> dict[str, Any]:
    ...
    """
    # Get basic schema
    schema = await _make_request(cluster, "GET", f"/druid/v2/datasources/{datasource}", ctx)
    ...
    columns_info = await _make_request(
        cluster,
        "POST",
        "/druid/v2/sql",
        ctx,
        json_data={"query": sql_query, "parameters": [{"type": "VARCHAR", "value": datasource}]},
    )
    ...
    new_dimensions = list(col_names.union(dimensions))

    schema["dimensions"] = new_dimensions
...
@mcp.tool()
async def list_supervisors(cluster: str, ctx: Context, include_state: bool = False) -> list[Any]:
    ...
    return await _make_request(cluster, "GET", "/druid/indexer/v1/supervisor", ctx, params=params)


@mcp.tool()
async def get_supervisor_status(cluster: str, supervisor_id: str, ctx: Context) -> dict[str, Any]:
    ...
    """
    return await _make_request(cluster, "GET", f"/druid/indexer/v1/supervisor/{supervisor_id}/status", ctx)
...
async def list_tasks(
    ...
    return await _make_request(cluster, "GET", "/druid/indexer/v1/tasks", ctx, params=params)


@mcp.tool()
async def get_task_status(cluster: str, task_id: str, ctx: Context) -> dict[str, Any]:
    ...
    """
    return await _make_request(cluster, "GET", f"/druid/indexer/v1/task/{task_id}/status", ctx)


@mcp.tool()
async def list_segments(cluster: str, datasource: str, ctx: Context, full: bool = False) -> list[Any]:
    ...
    return await _make_request(cluster, "GET", endpoint, ctx, params=params)
...
async def get_segments_info(cluster: str, datasource: str) -> dict[str, Any]:
    ...
    try:
        ...
        if not segments:
            return {"count": 0, "total_size": 0, "earliest": None, "latest": None}
...
@mcp.tool()
async def get_cluster_status(cluster: str, ctx: Context) -> dict[str, Any]:
    ...
    status["coordinator"] = await _check_leader_service(cluster, "/druid/coordinator/v1/leader", ctx)
    status["overlord"] = await _check_leader_service(cluster, "/druid/indexer/v1/leader", ctx)
...
@mcp.tool()
async def list_services(cluster: str, ctx: Context, service_type: str = "") -> list[dict[str, Any]]:
    ...
    if service_type:
        ...
        servers = await _make_request(
            cluster,
            "POST",
            "/druid/v2/sql",
            ctx,
            json_data={"query": query, "parameters": [{"type": "VARCHAR", "value": service_type}]},
        )
    else:
        query = "SELECT server, server_type as type, tier, curr_size, max_size FROM sys.servers"
        servers = await _make_request(cluster, "POST", "/druid/v2/sql", ctx, json_data={"query": query})

    return servers


@mcp.tool()
async def list_lookups(cluster: str, ctx: Context, tier: str = "") -> dict[str, Any]:
    ...
    return await _make_request(cluster, "GET", endpoint, ctx)


@mcp.tool()
async def get_lookup(cluster: str, lookup_id: str, ctx: Context, tier: str = "__default") -> dict[str, Any]:
    ...
    """
    return await _make_request(cluster, "GET", f"/druid/coordinator/v1/lookups/config/{tier}/{lookup_id}", ctx)


@mcp.tool()
async def get_lookup_status(cluster: str, ctx: Context, lookup_id: str = "", tier: str = "__default") -> dict[str, Any]:
    ...
    """
    status = await _make_request(cluster, "GET", "/druid/coordinator/v1/lookups/status", ctx)
...
@mcp.prompt()
def analyze_time_range(datasource: str, start_time: str = "", end_time: str = "") -> str:
